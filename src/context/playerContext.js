...
const PREFETCH_IMMEDIATE_DELAY_MS = 500; // Delay mínimo antes de iniciar prefetch (dejar que la canción actual arranque)
...
export const PlayerProvider = ({ children }) => {
    ...
    const prefetchAttemptsRef = useRef(0); // Contador de intentos para evitar loops infinitos
    ...
    useEffect(() => {
        ...
        // Volumen inicial
        if (playerARef.current) playerARef.current.volume = volume;
        if (playerBRef.current) playerBRef.current.volume = volume;
    ...
    }, []); // Run once (empty dep array importante)
    ...
    const playTrackInternal = useCallback(
        ...
        [resolveAudioUrl, showError, nextRequestId, syncIndicesByTrackId, clearPrefetch, checkIfAllQueueFailed]
    );
    ...
    const updateMediaSession = useCallback((track) => {
        ...
        const trackName = getSafeString(track.name || track.title);
        const artistName = getSafeString(track.artist);
        const albumName = getSafeString(track.album) || "Paradox Music";
